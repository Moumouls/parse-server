...
class SDLTransformer {
  ...
  async _prepare(){
    ...
    Object.keys(SDLSchema._typeMap).filter(
      ...
      Object.keys(SDLSchema._typeMap[type]._fields).forEach(fieldName => {
        this.types[type][fieldName] = {
          target: SDLSchema._typeMap[type]._fields[fieldName].type.name || SDLSchema._typeMap[type]._fields[fieldName].type.ofType.name,
          isArray: SDLSchema._typeMap[type]._fields[fieldName].type.name ? false : true
        }
      })
    })
  ...
  async _checkClasses(){
    ...
    const graphQLSystemClasses = systemClasses.map(systemClass => transformClassNameToGraphQL(systemClass))

    this.ops.classes.toDelete = this.parseClasses.filter(
      clazz =>
    ...
    this.parseClasses.forEach(clazz => {
      Object.keys(clazz.fields).forEach(fieldName =>{
        if(clazz.fields[fieldName].targetClass && this.ops.classes.toDelete.some(className => className === clazz.fields[fieldName].targetClass)){
          if(this.ops.fields.toDelete[clazz.className]) this.ops.fields.toDelete[clazz.className] = []
          this.ops.fields.toDelete[clazz.className].push(fieldName)
        }
      })
    })

    this.ops.classes.toAdd = Object.keys(this.types).filter(
      typeName => {
        const founded = this.parseClasses.some(clazz => transformClassNameToGraphQL(clazz.className) === typeName)
        if(!founded && graphQLSystemClasses.includes(typeName)) throw `Class ${typeName} is already reserved please change the type name`
        return !founded
      }
    );
