...
import { systemClasses, defaultColumns } from '../../Controllers/SchemaController'
...
const SDLTransformer = async (parseServer, typeDefs) => {
  ...
  const types = Object.keys(SDLSchema._typeMap).filter(type => !RESERVED_NATIVE_TYPE.includes(type))
...
const deleteClasses = async (classes, types, databaseController) => {
  const classesToDelete = classes.filter(clazz => !systemClasses.includes(clazz.className) && !types.includes(transformClassNameToGraphQL(clazz.className)))
  await Promise.all(classesToDelete.map(async (className) => databaseController.deleteSchema(className)))
}

const createOrUpdateClass = async (clazz, type) => {
  const typeFields = Object.keys(type._fields).map(fieldName => ({name: fieldName, type: JSON.stringify(type._fields[type._fields].type)}))
